...
#elif defined(NXP_SDK_CSE)
    #include "cse_driver.h"

#elif defined(NXP_SDK_HSE)
...
/* AES-DIRECT */
        ...
        void wc_AesEncryptDirect(Aes* aes, byte* out, const byte* in)
        {
            ...
            pKey = (uint8_t*)aes->key;
#endif
            if (STATUS_SUCCESS == CSE_DRV_LoadPlainKey(pKey, 10))
            {
                CSE_DRV_EncryptECB(CSE_RAM_KEY, in, AES_BLOCK_SIZE, out, CSE_TIMEOUT);
            }
        }
        ...
        void wc_AesDecryptDirect(Aes* aes, byte* out, const byte* in)
        {
            ...
            if (STATUS_SUCCESS == CSE_DRV_LoadPlainKey(pKey, 10))
            {
                CSE_DRV_DecryptECB(CSE_RAM_KEY, (uint8_t*)in, AES_BLOCK_SIZE, out, CSE_TIMEOUT);
            }
        }
#endif /* HAVE_AES_DECRYPT */
...
/* AES-CBC */
    ...
    int wc_AesCbcEncrypt(Aes* aes, byte* out, const byte* in, word32 sz)
    {
        ...
        if (STATUS_SUCCESS != CSE_DRV_EncryptCBC(CSE_RAM_KEY, in, sz, (uint8_t*)aes->reg, out, CSE_TIMEOUT))
        {
            ret = WC_TIMEOUT_E;
        }
        /* store iv for next call */
    ...
    int wc_AesCbcDecrypt(Aes* aes, byte* out, const byte* in, word32 sz)
    {
        ...
        if (STATUS_SUCCESS != CSE_DRV_DecryptCBC(CSE_RAM_KEY, (uint8_t*)in, sz, (uint8_t*)aes->reg, out, CSE_TIMEOUT))
        {
            ret = WC_TIMEOUT_E;
        }
        /* store iv for next call */
